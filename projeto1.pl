...
%areaIntersect(M,N,A)
%A: area da interseccao dos dois (se nao houver, sera 0)
areaIntersect(R1,R2,A) :-
	xEsq(R1,R2,X1,F1), ySup(R1,R2,Y1,F2),
	((F1<0 ; F2<0) -> A is 0;	xDir(R1,R2,X2), yInf(R1,R2,Y2), !, calcArea(X1,Y1,X2,Y2,A)).
